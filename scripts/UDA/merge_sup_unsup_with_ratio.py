...
original_unsup_data_path_2 = './new_test.txt'
augmented_unsup_data_path = './back_trans_data_remove_tag_dev/paraphrase/file_0_of_1.json'
augmented_unsup_data_path_2 = './back_trans_data_remove_tag_test/paraphrase/file_0_of_1.json'
combined_train_data_path = './combined_1_6_sup_train_data_unsup_dev_pubmed_data.json'
...
with open(original_unsup_data_path_2, 'r') as file:
    sents = []
    for line in file:
        if line.strip() != '':
            sents.append(line.strip())
...
with open(augmented_unsup_data_path_2, 'r') as file:
    sents = []
    for line in file:
        if line.strip() != '':
            sents.append(line.strip())
...
for i in range(len(sup_train_data_sentences)):
    ...
    abstract['ori_unsup_sentences'] = []
    abstract['aug_unsup_sentences'] = []
    for t in range(uda_ratio):
        abstract['ori_unsup_sentences'].append(ori_unsup_data_sentences[i * uda_ratio + t])
        abstract['aug_unsup_sentences'].append(aug_unsup_data_sentences[i * uda_ratio + t])
